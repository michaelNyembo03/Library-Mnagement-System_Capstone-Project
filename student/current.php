...
$rollno = $_SESSION['RollNo'];
    ...
    if (isset($_POST['submit'])) {$s = $_POST['title'];
        $sql = "select * from LMS.record,LMS.book where RollNo = '$rollno' and Date_of_Issue is NOT NULL and Date_of_Return is NULL and book.Bookid = record.BookId and (record.BookId='$s' or Title like '%$s%') order by Date_of_Issue DESC LIMIT $start_from, $record_per_page";

    } else {
        $sql = "select * from LMS.record,LMS.book where RollNo = '$rollno' and Date_of_Issue is NOT NULL and Date_of_Return is NULL and book.Bookid = record.BookId order by Date_of_Issue DESC LIMIT $start_from, $record_per_page";
    }
